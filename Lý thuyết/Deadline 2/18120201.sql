...
WHERE GV.GVQLCM = QLGV.MAGV AND QLGV.HOTEN = N'Nguyễn Hoài An' 
    AND THAMGIADT.MAGV = GV.MAGV AND THAMGIADT.MADT = DETAI.MADT
...
-- cách của anh nam đẹp zai: nhanh hơn 1 chút
SELECT DISTINCT GV.MAGV, GV.HOTEN
FROM GIAOVIEN AS GV, THAMGIADT, DETAI, BOMON
WHERE GV.MAGV = THAMGIADT.MAGV AND THAMGIADT.MADT = DETAI.MADT  -- tham gia đề tài 
    AND DETAI.GVCNDT = BOMON.TRUONGBM   -- trưởng bộ môn chủ trì đề tài 
    AND BOMON.MABM = GV.MABM    -- bộ môn của mình